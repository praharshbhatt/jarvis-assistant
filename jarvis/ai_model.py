...
class AIModel:
    ...
    def train(self, hard=False):
        ...
        self.custom_prompt = """
            ...
            {prompt}
            Give detailed answers to questions, based on the context provided.
    ...
    def query(self, query, should_speak=False):
        self.custom_chat_history.append(
            ChatMessage(role=MessageRole.USER, content=query)
        )

        response = self.chat_engine.chat(query).response
        self.custom_chat_history.append(
            ChatMessage(role=MessageRole.ASSISTANT, content=query)
        )
        # stream_chat = self.chat_engine.stream_chat(query)
        # response = stream_chat.response
        # for token in stream_chat.response_gen:
        #     print(token, end="")
        # return stream_chat.chat_stream
